            ...
            foreach (var i in methods)
            {
                ...
                for (int j = param.Length; j > 0; j--)
                {
                    ...
                    if (p.ParameterType.IsValueType && !p.ParameterType.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(p.ParameterType))
                    {
                        ...
                        if (isMultiArr)
                            sb.AppendLine(string.Format("                CSHotFix.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref a{0}, __intp, ptr_of_this_method, __mStack);", j, clsName));
                        else
                            sb.AppendLine(string.Format("                CSHotFix.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref @{0}, __intp, ptr_of_this_method, __mStack);", p.Name, clsName));
                ...
                if (!i.IsStatic)
                {
                    ...
                    else if (type.IsValueType && !type.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(type))
                    {
                        ...
                        sb.AppendLine(string.Format("                CSHotFix.Runtime.Generated.CLRBindings.s_{0}_Binder.ParseValue(ref instance_of_this_method, __intp, ptr_of_this_method, __mStack);", clsName));
